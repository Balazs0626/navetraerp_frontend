...
import { ArrowLeftOutlined, CheckCircleOutlined, ClockCircleOutlined, CloseCircleOutlined, PlusOutlined } from "@ant-design/icons";
import { useNavigate } from "react-router";
import { useInvalidate, useTranslation, usePermissions, useExport, CanAccess } from "@refinedev/core";
...
export const LeaveRequestList = () => {
  ...
  const { tableProps, setFilters, filters, result } = useTable<ILeaveRequestList>({
  ...
  useEffect(() => {
    document.title = `${translate("pages.leave_requests.list.title")} | NavetraERP`;
  })  
...
/*       exportFilters.push({
    ...
    getCheckboxProps: (record: ILeaveRequestList) => ({
      ...
      <List
        ...
        headerButtons={
          <Space>
            ...
            <Button icon={<PlusOutlined/>} size="large" type="primary" onClick={() => navigate("/hr/leave_requests/create")} disabled={!permissions?.includes("CREATE:LEAVE_REQUESTS")}>
              ...
              <Form form={form} layout="vertical" onFinish={onSearch}>
                <Row gutter={16}>
                  <Col span={24}>
                    ...
                    >
                      <Input placeholder={`${translate("pages.leave_requests.titles.employee")}...`} allowClear />
                    </Form.Item>
                  </Col>
                </Row>
